...
from indextts.gpt.model_vllm_v2_5 import UnifiedVoice
...
class IndexTTS2:
    def __init__(
        ...
        adapter_keys = set(self.gpt.state_dict())
        missing_checkpoint_keys = adapter_keys.difference(checkpoint)
        if missing_checkpoint_keys:
            missing_preview = ", ".join(sorted(missing_checkpoint_keys)[:10])
            raise RuntimeError(
                f"GPT checkpoint is missing adapter weights: {missing_preview}"
            )
        adapter_weights = {
            key: checkpoint[key] for key in adapter_keys
        }
        missing, unexpected = self.gpt.load_state_dict(
            adapter_weights, strict=True
        )
        if missing or unexpected:
            raise RuntimeError(
                "Failed to restore GPT adapter weights: "
                f"missing={missing}, unexpected={unexpected}"
            )
        del checkpoint, adapter_weights
        ...
        if use_deepspeed or use_accel:
            print(
                ">> --deepspeed/--accel only apply to the Transformers GPT "
                "backend and are ignored by vLLM"
            )
    ...
    # 原始推理模式
    async def infer(self, spk_audio_prompt, text, output_path, lang,
        ...
        if stream_return:
            ...
            async for result in self.infer_generator(
                spk_audio_prompt, text, output_path, lang,
                emo_audio_prompt, emo_alpha,
                emo_vector,
                use_emo_text, emo_text, use_random, interval_silence,
    ...
    async def infer_generator(self, spk_audio_prompt, text, output_path, lang,
        ...
        for seg_idx, text_tokens in enumerate(segment_tokens):
            ...
            # No torch mode context may span this await: those modes are
            # thread-local and concurrent asyncio requests share this thread.
            ...
            # CFM returns inference tensors. Consume them synchronously while
            # gradients are disabled, and leave this context before yielding.
            with torch.no_grad():
                m_start_time = time.perf_counter()
                diffusion_steps = 25
